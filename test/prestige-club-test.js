...
describe("PrestigeClub", function() {
  ...
  it2("Load Test", async function(){
    ...
    let one_ether = ethers.utils.parseEther("1000");
    let overrides = {
      value: one_ether
    }
    ...
    await signers[1]["recieve(address)"](accounts[0].address, {value: 100000});

    for(let i = 2 ; i < signers.length - 15 ; i++){
    ...
    for(let i = 0 ; i < signers.length - 15 ; i++){
      console.log("----");
      console.log("Stage: " + (await contract.users(accounts[i].address)).downlineBonus.toString())
      console.log("Payout: " + (await contract.getDownlinePayout(accounts[i].address)).toString())
      console.log("Volume: " + (await signers[i].getDownline())[0].toString())
    }

    // console.log((await signers[0].getDownlinePayout(accounts[0])).toString())
    // console.log((await signers[1].getDownlinePayout(accounts[1])).toString())
    // console.log((await signers[2].getDownlinePayout(accounts[2])).toString())
  ...
  it2("Downlinebonustest 2", async function(){
    ...
    // let balance = await ethers.getDefaultProvider().getBalance(accounts[1]);
  ...
  it2("Downlinebonus test", async function(){
    ...
    // await sleep((new Date().getTime() % 4000) + 100);

    const [accounts, contract] = await createContract("PrestigeClub");

    let [_, account1, account2, account3, account4, account5, account6] = getSigners(accounts, contract);
    
    let one_ether = ethers.utils.parseEther("100");//ethers.BigNumber.from(10000000000);
    let overrides = {
      value: one_ether
    }
    ...
    // await account1["triggerCalculation()"]();
    ...
    let userdata = await contract.users(accounts[1].address)
    expect(userdata.payout).satisfies(x => expected.fluctuation(x, 0.99))
    ...
    let userdata1 = await contract.users(accounts[1].address);
    ...
    expect(userdata1.payout).satisfies(x => expected1.fluctuation(x, 0.99))
  ...
  it2("Simple basic test", async function() {
    ...
    let one_ether = ethers.utils.parseEther("1");//ethers.BigNumber.from(10000000000);
    let overrides = {
      value: one_ether
    }
    ...
    expect(userdata["deposit"]).equal(base_deposit);
    expect((await contract.users(accounts[2].address)).deposit).equal(base_deposit);
    expect((await contract.users(accounts[1].address)).payout).equal(base_deposit.div(1000));
    ...
    expect((await contract.users(accounts[1].address)).payout).satisfies(x => (2 * expected + interest).fluctuation(x, 0.99));
